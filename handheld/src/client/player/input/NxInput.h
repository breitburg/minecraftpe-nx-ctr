#ifndef NET_MINECRAFT_CLIENT_PLAYER__SwitchInput_H__
#define NET_MINECRAFT_CLIENT_PLAYER__SwitchInput_H__
...
class NxTurnBuild : public UnifiedTurnBuild {
public:
	NxTurnBuild(int turnMode, int width, int height, float maxMovementDelta, float sensitivity, IInputHolder* holder, Minecraft* minecraft) :
...
class NxMoveInput : public KeyboardInput {
	typedef KeyboardInput super;
public:
	NxMoveInput(Options* options)
	:	super(options)
	{}
...
class NxInputHolder : public IInputHolder {
	static const int MovementLimit = 200; // per update

public:
	NxInputHolder(Minecraft* mc, Options* options) :
		_mc(mc),
		_move(options),
...
private:
	NxMoveInput _move;
	NxTurnBuild _turnBuild;
	Minecraft* _mc;
};

#endif /*NET_MINECRAFT_CLIENT_PLAYER__SwitchInput_H__*/